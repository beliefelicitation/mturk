...
if(chart.series[2])
{
	max_h = Math.max(Math.max(...chart.series[2].yData),max_h);
}
...
var chartoptions = {
    ...
    chart: {
        ...
        events: {
						...
						//this.yAxis[0].setExtremes(0,max_val);

						},
						render:function(e){

                        //$('#data').val(this.series[1].yData);
		...
		title: {
            text: ''
				...
				tickPositions : tickPositions(),
								labels: {
                formatter: function () {
                let tick =  min_tick + Math.round(100*this.value*(nb_bins-1)*step_tick)/100 ;
				if(prediction){
					if(tick >= 1000){
						return tick/1000+'k '+js_vars.xUnit;
					}else{
						return tick+' '+js_vars.xUnit;
...
var modelchartoptions = {
    ...
    chart: {
		...
		events:{
									load:function(e){
							let maxval = Math.max.apply(Math, model_data);
							if(maxval*zoom_area>1){
							this.yAxis[0].setExtremes(0,1);
							}else{
							this.yAxis[0].setExtremes(0,maxval*zoom_area);
...
function drawDefaultChart() {
    ...
    if(prediction!=true) {
		modelchart = new Highcharts.Chart('model', modelchartoptions);
	}
    if (interface == "ours"){
		settocurve();
		//$("#topchart").show(); //activate bins for ours interface
	}else{
		settobins();
}
}
...
if(interface=="metaculus") {
	...
	let slider_params = {
		start: [((max - min) * 0.25) + min, ((max - min) * 0.5) + min, ((max - min) * 0.75) + min],
		behaviour: "drag",
		range: {
			'min': min,
			'max': max
	...
	function createslider(nameid){
		...
		noUiSlider.create(slider, slider_params);
		means.push((max+min)/2);
		sigmas.push((max-min)/4);
		weights = [];
		for (var i=0;i<(id+1);i++) {
			weights.push(1 / (id + 1));
		}
		setgaussians(chart.series[1], means, sigmas, weights);
...
/*
const startleft=min+0*(max-min);
const startright=min+(max-min)*1;
*/
...
function update_gauss(){
	...
	//let maxaxis = Math.max.apply(Math, updatedgauss);
	//chart.yAxis[0].setExtremes(0, null,false,false);
/*chart.yAxis[0].update({
			tickPositions: [0, maxaxis]
	},false); */
...
$('#remove_component').on('click', function(){
 ...
 /*   if (new_val >= max_val)
	{chart.series[1].data[b].update(max_val,true,false,true);
	}else{
	chart.series[1].data[b].update(new_val,true,false,true);
		}
*/
});
